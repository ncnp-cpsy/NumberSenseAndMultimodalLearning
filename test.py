...
from src.config_test import (
    config_test_trainer_vae_cmnist,
    config_test_trainer_vae_oscn,
    config_test_trainer_mmvae_cmnist_oscn,
    config_test_analyzer_vae_cmnist,
    config_test_analyzer_vae_oscn,
    config_test_analyzer_mmvae_cmnist_oscn,
    config_test_classifier_cmnist,
    config_test_classifier_oscn,
...
def test_train_classifier():
    ...
    args = config_test_classifier_oscn
    args.pretrained_path = ''
    main(args=args)
    return

def test_train():
    args = config_test_trainer_vae_cmnist
    args.run_id = 'test_vae_cmnist'
    main(args=args)

    args = config_test_trainer_vae_oscn
    args.run_id = 'test_vae_oscn'
    main(args=args)

    args = config_test_trainer_mmvae_cmnist_oscn
    args.run_id = 'test_mmvae_cmnist_oscn'
    main(args=args)
    return

def test_analyse():
    experiment_name = config_test_trainer_vae_oscn.experiment

    # CMNIST
    args = config_test_analyzer_vae_cmnist
    ...
    main(
        args=args,
        args_classifier_cmnist=config_test_classifier_cmnist,
        args_classifier_oscn=config_test_classifier_oscn,
    )

    # OSCN
    args = config_test_analyzer_vae_oscn
    model_name = 'VAE_OSCN'
    args.run_id = 'test_vae_oscn'
    args.pretrained_path = os.path.join(
        './rslt', experiment_name, model_name, args.run_id)
    ...
    # MMVAE
    args = config_test_analyzer_mmvae_cmnist_oscn
...
def test_train_loop():
    ...
    for seed in range(3, 5):
        args = copy.copy(config_test_trainer_vae_cmnist)
        args.seed = seed
        ...
        args = copy.copy(config_test_trainer_vae_oscn)
        args.seed = seed
        ...
        # MMVAE
        args = copy.copy(config_test_trainer_mmvae_cmnist_oscn)
        args.seed = seed
...
def test_analyse_loop(run_ids_dict):
    experiment_name = config_test_trainer_vae_cmnist.experiment
    for model_name in run_ids_dict.keys():
        for run_id in run_ids_dict[model_name]:
            if model_name == 'VAE_OSCN':
                args = copy.copy(config_test_analyzer_vae_oscn)
            elif model_name == 'VAE_CMNIST':
                args = copy.copy(config_test_analyzer_vae_cmnist)
            elif model_name == 'MMVAE_CMNIST_OSCN':
                args = copy.copy(config_test_analyzer_mmvae_cmnist_oscn)
            else:
                Exception
            args.run_id = run_id
            args.pretrained_path = os.path.join(
...
def test_synthesize():
    args = copy.copy(config_test_synthesizer)
    experiment_name = config_test_trainer_vae_cmnist.experiment
    model_name = 'VAE_OSCN'
