...
def unpack_data(dataB, device='cuda', require_label = False, target = 0):
    ...
    if is_multidata(dataB):
        if torch.is_tensor(dataB[0]):
            if torch.is_tensor(dataB[1]):
                ...
                if require_label:
                    return dataB[0].to(device),  dataB[1]
                else:
                    return dataB[0].to(device) # cubISft
            else:
            ...
            if require_label:
                #print(dataB[0][1])
                #dataB[0][1] = torch.nn.functional.one_hot(dataB[0][1], num_classes=10).float()
                try:
                    #converted_oscn_label = convert_oscn_label(dataB[1][1])
                
                    return [d.to(device) for d in list(zip(*dataB))[0]], [dataB[0][1], dataB[1][1]]  # mnist-svhn, cubIS
                except:
                    return [d.to(device) for d in list(zip(*dataB))[0]], [np.array(dataB[0][1]), np.array(dataB[0][1])] 
...
def convert_oscn_label(raw_label):
    col_dict = {
        "r" : 0,
        "g" : 1,
        "b" : 2,
    ...
    col_labels = [col_dict[l[0]] for l in list(raw_label)]
    ...
    col_labels = [col_dict[l[2]] for l in list(raw_label)] """
    return col_labels
